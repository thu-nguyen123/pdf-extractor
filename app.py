...
def extract_trading(pages):
    ...
    for page_num, text in pages:
        ...
        # FIX: lấy total từ toàn page
        total_cartons = get_value(r'Total Number of Cartons:\s*(\d+)', text)
        total_qty = get_value(r'Total Invoice Quantity:\s*([\d,]+)', text)
        total_amount = get_value(r'Total Amount:\s*([\d,.]+)', text)
        gross_weight = get_value(r'Total Gross Weight\s*:\s*([\d.]+)', text)
